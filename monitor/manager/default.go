...
func (m *manager) Check(service string) error {
...
func (m *manager) check(service string) (*Status, error) {
...
func (m *manager) Reap(service string) error {
	services, err := m.registry.GetService(service)
	if err != nil {
		return nil
	}
...
func (m *manager) Watch(service string) error {
	m.Lock()
	defer m.Unlock()
