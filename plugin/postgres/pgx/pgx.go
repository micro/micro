...
func (s *sqlStore) List(opts ...store.ListOption) ([]string, error) {
	...
	var rows pgx.Rows
	if options.Limit > 0 {
		if options.Order == store.OrderAsc {
			rows, err = db.Query(s.options.Context, queries.ListAscLimit, pattern, options.Limit, options.Offset)
		} else {
			rows, err = db.Query(s.options.Context, queries.ListDescLimit, pattern, options.Limit, options.Offset)
		}
	} else {
		if options.Order == store.OrderAsc {
			rows, err = db.Query(s.options.Context, queries.ListAsc, pattern)
		} else {
			rows, err = db.Query(s.options.Context, queries.ListDesc, pattern)
		}
	}
	if err != nil {
