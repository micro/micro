...
func (c *Converter) ConvertFrom(rd io.Reader) (*plugin.CodeGeneratorResponse, error) {
	...
	if err != nil {
		c.logger.Error("Failed to read request: %v", err)
		return nil, err
	}
	...
	if err != nil {
		c.logger.Error("Can't unmarshal input: %v", err)
		return nil, err
	}
...
func (c *Converter) convertFile(file *descriptor.FileDescriptorProto) error {
	...
	for _, msg := range file.GetMessageType() {
		...
		c.logger.Infof("Generating schema for message (%s) from proto file (%s)", msg.GetName(), protoFileName)
