...
func (r *Router) Lookup(ctx context.Context, req *pb.LookupRequest, resp *pb.LookupResponse) error {
	...
	if err == router.ErrRouteNotFound {
		return errors.NotFound("router", err.Error())
	} else if err != nil {
		return errors.InternalServerError("router", "failed to lookup routes: %v", err)
	}
...
func (r *Router) Advertise(ctx context.Context, req *pb.Request, stream pb.Router_AdvertiseStream) error {
	...
	if err != nil {
		return errors.InternalServerError("router", "failed to get adverts: %v", err)
	}
	...
	for advert := range advertChan {
		...
		if err != nil {
			return errors.InternalServerError("router", "error sending message %v", err)
		}
	}
...
func (r *Router) Process(ctx context.Context, req *pb.Advert, rsp *pb.ProcessResponse) error {
	...
	if err := r.Router.Process(advert); err != nil {
		return errors.InternalServerError("router", "error publishing advert: %v", err)
	}
...
func (r *Router) Watch(ctx context.Context, req *pb.WatchRequest, stream pb.Router_WatchStream) error {
	...
	if err != nil {
		return errors.InternalServerError("router", "failed creating event watcher: %v", err)
	}
	defer watcher.Stop()
	defer stream.Close()
	...
	for {
		...
		if err == router.ErrWatcherStopped {
			return errors.InternalServerError("router", "watcher stopped")
		}

		if err != nil {
			return errors.InternalServerError("router", "error watching events: %v", err)
		}
