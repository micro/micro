...
func (a *Auth) Generate(ctx context.Context, req *pb.GenerateRequest, rsp *pb.GenerateResponse) error {
	...
	if len(req.Id) == 0 {
		return errors.BadRequest("auth", "ID required")
	}
...
func (a *Auth) createAccount(acc *auth.Account) error {
	...
	if err != nil {
		return errors.InternalServerError("auth", "Unable to hash password: %v", err)
	}
	acc.Secret = secret
	...
	if err != nil {
		return errors.InternalServerError("auth", "Unable to marshal json: %v", err)
	}

	// write to the store
	if err := store.Write(&gostore.Record{Key: key, Value: bytes}); err != nil {
		return errors.InternalServerError("auth", "Unable to write account to store: %v", err)
	}

	// set a refresh token
	if err := a.setRefreshToken(acc.Issuer, acc.ID, uuid.New().String()); err != nil {
		return errors.InternalServerError("auth", "Unable to set a refresh token: %v", err)
	}
...
func (a *Auth) Inspect(ctx context.Context, req *pb.InspectRequest, rsp *pb.InspectResponse) error {
	...
	if err == token.ErrInvalidToken || err == token.ErrNotFound {
		return errors.BadRequest("auth", err.Error())
	} else if err != nil {
		return errors.InternalServerError("auth", "Unable to inspect token: %v", err)
	}
...
func (a *Auth) Token(ctx context.Context, req *pb.TokenRequest, rsp *pb.TokenResponse) error {
	...
	if (len(req.Id) == 0 || len(req.Secret) == 0) && len(req.RefreshToken) == 0 {
		return errors.BadRequest("auth", "Credentials or a refresh token required")
	}
	...
	if len(req.RefreshToken) > 0 {
		...
		if err == gostore.ErrNotFound {
			return errors.BadRequest("auth", "Account can't be found for refresh token")
		} else if err != nil {
			return errors.InternalServerError("auth", "Unable to lookup token: %v", err)
		}
		accountID = accID
	}
	...
	if err == gostore.ErrNotFound {
		return errors.BadRequest("auth", "Account not found with this ID")
	} else if err != nil {
	...
	if err := json.Unmarshal(recs[0].Value, &acc); err != nil {
		return errors.InternalServerError("auth", "Unable to unmarshal account: %v", err)
	}
	...
	if len(req.RefreshToken) == 0 {
		if !secretsMatch(acc.Secret, req.Secret) {
			return errors.BadRequest("auth", "Secret not correct")
		}

		refreshToken, err = a.refreshTokenForAccount(req.Options.Namespace, acc.ID)
		if err != nil {
			return errors.InternalServerError("auth", "Unable to get refresh token: %v", err)
		}
	}
	...
	if err != nil {
		return errors.InternalServerError("auth", "Unable to generate token: %v", err)
	}
