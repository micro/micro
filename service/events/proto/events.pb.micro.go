...
func (c *brokerService) Publish(ctx context.Context, in *PublishRequest, opts ...client.CallOption) (*PublishResponse, error) {
...
func (x *brokerServiceSubscribe) Recv() (*Event, error) {
	m := new(Event)
	err := x.stream.Recv(m)
	if err != nil {
		return nil, err
...
func (h *brokerHandler) Publish(ctx context.Context, in *PublishRequest, out *PublishResponse) error {
...
func (x *brokerSubscribeStream) Send(m *Event) error {
	return x.stream.Send(m)
}
