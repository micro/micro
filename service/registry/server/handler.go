...
func (r *Registry) GetService(ctx context.Context, req *pb.GetRequest, rsp *pb.GetResponse) error {
	...
	if err := namespace.Authorize(ctx, options.Domain, publicNS); err == namespace.ErrForbidden {
		return errors.Forbidden("registry", err.Error())
	} else if err == namespace.ErrUnauthorized {
		return errors.Unauthorized("registry", err.Error())
	} else if err != nil {
		return errors.InternalServerError("registry", err.Error())
	}
	...
	if err == goregistry.ErrNotFound {
		return errors.NotFound("registry", err.Error())
	} else if err != nil {
		return errors.InternalServerError("registry", err.Error())
	}
