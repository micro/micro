...
func (n *net) Init(opts ...network.Option) error {
	n.Lock()
	defer n.Unlock()
...
func (n *net) Options() network.Options {
	n.RLock()
	defer n.RUnlock()
...
func (n *net) acceptNetConn(l tunnel.Listener, recv chan *message) {
	var i int
	for {
		// accept a connection
		conn, err := l.Accept()
		if err != nil {
			sleep := backoff.Do(i)
			logger.Debugf("Network tunnel [%s] accept error: %v, backing off for %v", network.ControlChannel, err, sleep)
			time.Sleep(sleep)
			i++
			continue
		}
		...
		case <-n.closed:
			if err := conn.Close(); err != nil {
				logger.Debugf("Network tunnel [%s] failed to close connection: %v", network.NetworkChannel, err)
			}
			return
		default:
...
func (n *net) acceptCtrlConn(l tunnel.Listener, recv chan *message) {
	var i int
	for {
		// accept a connection
		conn, err := l.Accept()
		...
		case <-n.closed:
			if err := conn.Close(); err != nil {
				if logger.V(logger.DebugLevel, logger.DefaultLogger) {
					logger.Debugf("Network tunnel [%s] failed to close connection: %v", network.ControlChannel, err)
				}
			}
			return
		default:
...
func (n *net) maskRoute(r *pbRtr.Route) {
	hasher := fnv.New64()
	...
	r.Address = address
	r.Gateway = n.Address()
	r.Link = network.DefaultLink
	r.Metric = metric
}

// advertise advertises routes to the network
func (n *net) advertise(advertChan <-chan *router.Advert) {
		...
		case advert := <-advertChan:
			...
			for i := 0; i < max; i++ {
				if peer := n.node.GetPeerNode(peers[rnd.Intn(len(peers))].Id()); peer != nil {
					if err := n.sendTo("advert", network.ControlChannel, peer, msg); err != nil {
						if logger.V(logger.DebugLevel, logger.DefaultLogger) {
...
func (n *net) initNodes(startup bool) {
...
func (n *net) resolveNodes() ([]string, error) {
...
func (n *net) handleNetConn(s tunnel.Session, msg chan *message) {
	for {
		m := new(transport.Message)
		if err := s.Recv(m); err != nil {
			if logger.V(logger.DebugLevel, logger.DefaultLogger) {
				logger.Debugf("Network tunnel [%s] receive error: %v", network.NetworkChannel, err)
			}
			switch err {
...
func (n *net) handleCtrlConn(s tunnel.Session, msg chan *message) {
	for {
		m := new(transport.Message)
		if err := s.Recv(m); err != nil {
			if logger.V(logger.DebugLevel, logger.DefaultLogger) {
				logger.Debugf("Network tunnel [%s] receive error: %v", network.ControlChannel, err)
			}
			switch err {
			case io.EOF, tunnel.ErrReadTimeout:
				s.Close()
...
func (n *net) getHopCount(rtr string) int {
...
func (n *net) getRouteMetric(router string, gateway string, link string) int64 {
...
func (n *net) processCtrlChan(listener tunnel.Listener) {
...
func (n *net) processNetChan(listener tunnel.Listener) {
	defer listener.Close()
		...
		case m := <-recv:
			...
			case "connect":
				...
				if err := proto.Unmarshal(m.msg.Body, pbNetConnect); err != nil {
					logger.Debugf("Network tunnel [%s] connect unmarshal error: %v", network.NetworkChannel, err)
					continue
				}
				...
				if err := n.AddPeer(peer); err == network.ErrPeerExists {
				...
				go func() {
					...
					node := util.PeersToProto(n.node, network.MaxDepth)
					...
					if err := n.sendTo("sync", network.NetworkChannel, peer, msg); err != nil {
			...
			case "peer":
				...
				if err := proto.Unmarshal(m.msg.Body, pbNetPeer); err != nil {
					logger.Debugf("Network tunnel [%s] peer unmarshal error: %v", network.NetworkChannel, err)
					continue
				}
				...
				} else if err != network.ErrPeerExists {
				...
				peer = UnpackPeerTopology(pbNetPeer, now, network.MaxDepth-1)
				// update the link
			...
			case "sync":
				...
				if err := proto.Unmarshal(m.msg.Body, pbNetSync); err != nil {
					logger.Debugf("Network tunnel [%s] sync unmarshal error: %v", network.NetworkChannel, err)
					continue
				}
				...
				// add peer to the list of node peers
				if err := n.node.AddPeer(peer); err == network.ErrPeerExists {
					if logger.V(logger.TraceLevel, logger.DefaultLogger) {
						logger.Tracef("Network peer exists, refreshing: %s", peer.id)
					}
				...
				for _, pbRoute := range pbNetSync.Routes {
					...
					route := util.ProtoToRoute(pbRoute)
				...
				go func() {
					...
					msg := util.PeersToProto(n.node, network.MaxDepth)

					// advertise yourself to the new node
					if err := n.sendTo("peer", network.NetworkChannel, peer, msg); err != nil {
						if logger.V(logger.DebugLevel, logger.DefaultLogger) {
			...
			case "close":
				...
				if err := proto.Unmarshal(m.msg.Body, pbNetClose); err != nil {
					if logger.V(logger.DebugLevel, logger.DefaultLogger) {
						logger.Debugf("Network tunnel [%s] close unmarshal error: %v", network.NetworkChannel, err)
					}
					continue
				}
...
func (n *net) pruneRoutes(q ...router.QueryOption) error {
	routes, err := n.router.Table().Query(q...)
	if err != nil && err != router.ErrRouteNotFound {
		return err
	}
...
func (n *net) prunePeerRoutes(peer *node) error {
...
func (n *net) manage() {
	rnd := rand.New(rand.NewSource(time.Now().UnixNano()))
	announce := time.NewTicker(network.AnnounceTime)
	defer announce.Stop()
	prune := time.NewTicker(network.PruneTime)
	defer prune.Stop()
	resolve := time.NewTicker(network.ResolveTime)
	defer resolve.Stop()
	netsync := time.NewTicker(network.SyncTime)
	defer netsync.Stop()
	...
	for {
		...
		case <-announce.C:
			...
			for _, peer := range n.peers {
				...
				if lastSent.IsZero() || time.Since(lastSent) > network.KeepAliveTime {
					link := peer.link
					id := peer.id
			...
			for link, lastSent := range links {
				...
				if err := n.sendTo("connect", network.NetworkChannel, peer, msg); err != nil {
					if logger.V(logger.DebugLevel, logger.DefaultLogger) {
		...
		case <-prune.C:
			...
			pruned := n.PruneStalePeers(network.PruneTime)
...
func (n *net) getProtoRoutes() ([]*pbRtr.Route, error) {
	// get a list of the best routes for each service in our routing table
	q := []router.QueryOption{
		router.QueryStrategy(n.router.Options().Advertise),
	}
	...
	for _, route := range routes {
		...
		pbRoute := util.RouteToProto(route)
...
func (n *net) sendConnect() {
	...
	if err := n.sendMsg("connect", network.NetworkChannel, msg); err != nil {
		if logger.V(logger.DebugLevel, logger.DefaultLogger) {
...
func (n *net) sendTo(method, channel string, peer *node, msg proto.Message) error {
	...
	if err != nil {
		if peerNode := n.GetPeerNode(peer.id); peerNode != nil {
			...
			if count := peerNode.status.Error().Count(); count == network.MaxPeerErrors {
				if logger.V(logger.DebugLevel, logger.DefaultLogger) {
					logger.Debugf("Network peer %v error count exceeded %d. Prunning.", peerNode, network.MaxPeerErrors)
				}
	...
	if err := c.Send(tmsg); err != nil {
		...
		if peerNode := n.GetPeerNode(peer.id); peerNode != nil {
			...
			if count := peerNode.status.Error().Count(); count == network.MaxPeerErrors {
				if logger.V(logger.DebugLevel, logger.DefaultLogger) {
					logger.Debugf("Network node peer %v count exceeded %d: %d", peerNode, network.MaxPeerErrors, peerNode.status.Error().Count())
				}
				n.PrunePeer(peerNode.id)
			}
		}
...
func (n *net) sendMsg(method, channel string, msg proto.Message) error {
	body, err := proto.Marshal(msg)
	if err != nil {
		return err
	}
	...
	if !ok || client == nil {
		n.RUnlock()
		return network.ErrClientNotFound
	}
	n.RUnlock()
...
func (n *net) updatePeerLinks(peer *node) error {
	n.Lock()
	defer n.Unlock()
	...
	if peerLink == nil {
		return network.ErrPeerLinkNotFound
	}
...
func (n *net) isLoopback(link tunnel.Link) bool {
	// skip loopback
	if link.Loopback() {
		return true
	}
...
func (n *net) Connect() error {
	...
	n.tunClient[network.ControlChannel] = ctrlClient
	...
	n.tunClient[network.NetworkChannel] = netClient
...
func (n *net) createClients() error {
	// dial into ControlChannel to send route adverts
	ctrlClient, err := n.tunnel.Dial(network.ControlChannel, tunnel.DialMode(tunnel.Multicast))
	if err != nil {
		return err
	}

	// dial into NetworkChannel to send network messages
	netClient, err := n.tunnel.Dial(network.NetworkChannel, tunnel.DialMode(tunnel.Multicast))
	if err != nil {
		return err
	}
	...
	c, ok := n.tunClient[network.ControlChannel]
	if ok {
		c.Close()
	}
	n.tunClient[network.ControlChannel] = ctrlClient
	...
	c, ok = n.tunClient[network.NetworkChannel]
	if ok {
		c.Close()
	}
	n.tunClient[network.NetworkChannel] = netClient
...
func (n *net) Close() error {
	...
	default:
		...
		if err := n.sendMsg("close", network.NetworkChannel, msg); err != nil {
			if logger.V(logger.DebugLevel, logger.DefaultLogger) {
...
func (n *net) Client() client.Client {
	return n.client
}

// Server returns network server
func (n *net) Server() server.Server {
	return n.server
}
